/**
 * 画像ビーコンの受け口 (`/v1/probe.gif`・`/v1/p.gif`・`/v1/enroll.gif`) が返す応答。
 *
 * **どれもキャッシュさせない** (`no-store`)。送るたびに URL が変わるので効かないが、途中で保存させない意思を示す。
 * 結果は GIF の幅で返す (UserScript は `<img>` の `naturalWidth` しか読めない)。
 */

/** 200 と幅 `width` の透過 GIF。 */
export function gifResponse(width: number): Response {
...
/** 拒否。**画像を返さない**ので、クライアントは `onerror` で失敗を知る。 */
export function plainResponse(status: keyof typeof STATUS_TEXT): Response {
