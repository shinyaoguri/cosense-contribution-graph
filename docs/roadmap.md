...
2. 登録済みのブラウザ A で、プロジェクト P と Q で数分ずつ読み書きしてタブを隠す → 「草: センサーの記録」で
   「最後の送信: …タブを隠したとき — 書いた」「今日の送信: 1 / 4 回」「まだ送れていない日: 0 日」
3. すぐもう一度隠す → 変化が無ければ回数が増えない
4. ブラウザ B (同じアカウントで登録済み) で、A と**同じ時刻の分**と違う時刻の分を作って隠す
5. レポートの合算の草の URL に `?r=<任意>` を付けて開き、今日のマスに色が出る。**重なった分が二重に数えられていない** (A と B の分数の和より少ない)
...
- 残り: ダイアログをページのテーマに合わせる (#81)、ページの見た目 (#92)。
  「草の設定」(#79) と「ここから計測開始」の印 (#80) は済み
- **ページメニューを 1 項目に畳んで `v1` を配れる形にした** (2026-09-15、Issue #95)。
  `USERSCRIPT_VERSION` は `1.0.0`。**`dev` には貼った** (2026-09-15、手元の `cosense` CLI。
  ADR-0013 決定 3 の改訂)。`v1` は dev で確かめてから貼る
- 段階 6 の確かめ方の手順 4 (プロジェクト別の草) も、「草を見る」で見られる

### 「草の設定」(2026-09-15 着手、Issue #79)

ページメニュー「草の設定」のダイアログ (`settings.ts` がモデル、`settings-dialog.ts` が DOM。「草を見る」と同じ分け方)。
...
- **サインインは単独のメニューをやめ、ここに集約した** (design §9)。**ページメニューに足すのは
  「草を見る」の 1 項目だけで、「草の設定」はそのダイアログの下のボタンから開く** (2026-09-15、Issue #95)。
...
| 段階 5 | **Cosense の配布用公開プロジェクト** | **用意済み (2026-09-14、[`/cosense-grass`](https://scrapbox.io/cosense-grass/))。** ページは `dev` (開発版) と `v1`… (リリース版)。**`dev` と `v1` には同じバンドルを貼る** (2026-09-15、Issue #95。ADR-0005 の改訂)。貼るのは手元の `cosense` CLI で、CI では自動化しない (ADR-0013 決定 3 の改訂)。メンバーは持ち主だけ |
