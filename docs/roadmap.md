...
1. 配布ページにバンドルを貼り、自分のページで「草: 記録の疎通確認」を押す。鍵 (IndexedDB) と試験用の uid (localStorage) が
...
- **Cosense のページに `[ ]` で貼ったときの見た目** (ライト・ダーク)。応答のヘッダはデモ (#20 で表示を確認済み) と同じ `svgResponse` から出ている
