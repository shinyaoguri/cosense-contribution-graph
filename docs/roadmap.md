...
設計は `design.md`、判断の根拠は `decisions/`、前提となる実測事実は `research.md` にある。
...
**1 回目の brand verification は差し戻された** (2026-09-24)。指摘と対処。

| 指摘 | 対処 |
|---|---|
| ホームページのウェブサイトが登録されていない (所有権) | 持ち主の操作。`soui.dev` の TXT は DNS にあるので、**Search Console でドメイン プロパティを「確認」まで済ませ、そのアカウントが GCP プロジェクトの Owner か Editor であること**を見る |
| ポリシーの内容が足りない | `docs/privacy.md` に「Google ユーザーデータの取り扱い」(取得・使い方・共有・保護・保持と削除) と英語の要約。`<h1>` にアプリ名 |
| ホームページがログインページに見える | サインインのリンクを冒頭から「使い方」の後ろへ移した |
| ホームページに目的が無い | 「Google サインインを使う理由」と英語の節 `About cosense-grass` |
| アプリ名がホームページと一致しない | `<h1>cosense-grass</h1>` はあったが日本語だけだった。英語の節の本文にも `cosense-grass` を出す |

ページの修正が本番に出てから再審査に出す。

**2 回目も差し戻された** (2026-09-24)。所有権の指摘は消えた (別のアプリのために別のアカウントで確認していたので、
GCP プロジェクトの Owner のアカウントで TXT を**追加**して確認済みオーナーを足した。既存の TXT は消さない)。
残りの 4 点は同じ文面で返ってきた。本番のページは要件を満たしていて、自動判定の誤りの可能性が高い (research §6)。
それでも材料になりうるところは潰した (Issue #141)。

- トップの最初の本文を英語の `About cosense-grass` にした (`<h1>` の直後)
- `<title>` をアプリ名だけにし、`application-name` と `description` の meta を足した
- トップから `/auth/start` への直接のリンクをなくし、`/account` (未サインインならサインインを促す) へ案内する

**トップとポリシーを英語のページにした** (2026-09-26、ADR-0022)。登録済みの `/` と `/privacy` を英語だけにし、
日本語は `/ja` と `/ja/privacy` に分けて上端のリンクで行き来する。URL は変えていないので同意画面の設定は触らない。

**Publish app は brand verification を待たないので、先に押してよい。** 審査が通るまでは同意画面に名前とロゴの代わりにドメインが出る。
再審査は、キャッシュした判定を避けるため少し時間を置いてから出す。それでも同じなら、審査のメールへの返信か
Google Cloud のサポートで人の確認を頼む。
...
**決めた値と、そう決めた根拠を `decisions/` に ADR として残す。** 後から触るときに
「なぜこの数字か」が読めなくなるのを防ぐ。
...
- ~~`src/userscript/render.ts` 草とツールチップ。合算 1 枚 + プロジェクト別を並べる~~
  **1.1.0 で撤去** (ADR-0019。草を描くのは Worker だけ)
...
- **ページメニューを 1 項目に畳んで `v1` を配れる形にした** (2026-09-15、Issue #95)。
  ...
  ADR-0013 決定 3 の改訂)。どちらも配信されている中身が手元の `dist/userscript.js` と一致する
  (**末尾の改行 1 つを落としてから比べる**。Cosense は行の配列なので配信物に改行が無い)
- **貼り付けをスクリプトにした** (2026-09-15、`scripts/paste-distribution.ts`、Issue #105)。
  `npm run paste -- <page>`。ops を手で組んで貼ったときに分割の順番を間違え、`dev` を 40 秒空にしたため。
  **消す行を挿入より前に確定することを仕組みで縛る** (再開するときも、そのとき決めた一覧を持ち越す)。
  あわせて `check-distribution.sh` が末尾の改行で永久に「違う」と言っていたのも直した
  (#60 が閉じないままだった原因)
...
- **ローカル描画を全廃した** (2026-09-15、Issue #109。ADR-0019)。`render.ts` と `viewer.ts` の
  `describeLocal`、localStorage の日次集計 (`cosense-grass:daily`) を撤去し、草は共有 SVG だけにした。
...
**2026-09-26 着手、Issue #148。** GitHub の Activity overview にならい、書いた分を「作る / 育てる / 関わる」に分け、
「読む」と合わせた 4 軸のレーダーを草とは別の SVG で出す (ADR-0021、design §4)。**草の SVG は変えない。**

| 順 | Issue | 中身 | 前提 |
|---|---|---|---|
| 1 | #149 | ADR を `docs/decisions/` へ分割 (**済み**) | |
| 2 | #150 | ADR-0021・設計・ポリシーと、センサーの前提の実測 (**済み**、2026-09-26) | #149 |
| 3 | #151 | ビーコンを v2 に (`wc` / `wo` / `links`)、D1 のマイグレーション 0005、`store.ts` の v2。センサーはまだ 0 を送る (**済み**) | #150 |
| 4a | #152 | センサーが書いたページを REST で作る・関わる・育てるに振り分ける (design §9) (**済み**) | #151 |
| 4b | #153 | `/v1/g/{publicId}/overview.svg` (design §6・§8) (**済み**) | #151 |
| 5 | #154 | ダイアログ・`/account`・トップに並べる (**済み**)。配布は持ち主の指示で `dev` にだけ貼る (2026-09-26)。`v1` の扱いと v=1 の停止は #159 | #152, #153 |
| 後 | #155 | 作ったリンクの数 (`links`)。`scrapbox.Page.lines` の `nodes` から数える (research §2) | #151 |

### 実測で決まったこと (2026-09-26、research §2・§4)

- **作成者は REST `/api/pages/v2/:project/:title` の `user`、作成時刻は `created`。** 振り分けはこの 2 つで決め、`persistent` は使わない。
  プランの「`persistent` が false なら新規」から変えた (ADR-0021 決定 3)
- プレースホルダーの `scrapbox.Page.id` が保存後も変わらないことは傍証だけ。振り分けは ID に依らない
- `scrapbox.Page.lines` の記法を含む行は `nodes` (構文木) を持つ。リンクは `type: "link"`

### 完了条件

- 草を貼ったページの見た目が変わらない (草のゴールデンテストが通ったまま)
- `/v1/g/demo/overview.svg` がデモの 4 軸を描く
- 本番の JSON に `wc` / `wo` / `links` が出て、自分の端末で作る・関わるの分が 0 でない日がある
...
- ~~`docs/decisions.md` を `docs/decisions/NNNN-<slug>.md` に分割するか。~~ **分割した** (2026-09-26、Issue #149)。
  ADR-0021 を足す前に、20 件に達したところで分けた。`docs/decisions.md` は古いリンクの受け皿として残している
