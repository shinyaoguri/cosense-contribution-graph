...
| 3 | #151 | ビーコンを v2 に (`wc` / `wo` / `links`)、D1 のマイグレーション 0005、`store.ts` の v2。センサーはまだ 0 を送る | #150 |
| 4a | #152 | センサーが書いたページを REST で作る・関わる・育てるに振り分ける (design §9) | #151 |
| 4b | #153 | `/v1/g/{publicId}/overview.svg` (design §6・§8) | #151 |
| 5 | #154 | ダイアログ・`/account`・トップに並べる。**配布ページ (`v1` を貼り替えるか `v2` を出すか) は持ち主に確かめる** | #152, #153 |
