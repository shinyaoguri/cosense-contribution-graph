...
```
uid        HMAC-SHA256(WORKER_SECRET, "google:" + sub)   サーバ主キー。外に出さない
...
```sql
CREATE TABLE users (
  uid       TEXT PRIMARY KEY,            -- HMAC-SHA256(WORKER_SECRET, "google:" + sub)
...
1. `/api/pages/:project?limit=1000&skip=N` でページ一覧を取得 (limit は 1000 でクランプされる)
...
- `shared/ids.ts` `ph` が uid でソルトされていること。プロジェクト別 publicId から uid も
