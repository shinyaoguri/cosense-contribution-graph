...
src/userscript/
  ...
  graph-dialog.ts           「草を見る」のダイアログ。全端末を統合した草を img で並べる。「草の設定」への導線もここ
  render.ts                 このブラウザの記録から描く草とツールチップ (計測開始前は点線の枠)
...
- **`<img>` 内の SVG は完全に非インタラクティブ。** `<title>` のツールチップも出ず (「草を見る」のこのブラウザの記録は、ページの中の SVG なので出る。§9)、外部フォントも
...
- **過去の未送信の日は、どのきっかけでも送る。** 今日を送るのは隠したときと登録の成功のときだけ
...
**ページメニューに足すのは「草を見る」の 1 項目だけ** (2026-09-15、Issue #95)。ほかのスクリプトと並ぶ場所なので占有を最小にし、
「草の設定」はそのダイアログの下のボタンから開く。**「草を見る」は `<dialog>` で開く** (ADR-0003 の 2026-09-15 の改訂)。
...
- **このブラウザから 1 件も送れていないプロジェクトは、「表示してみる」を押すまで画像を読まない。** 404 のリクエストを出さず、
  URL をコンソールのエラーに残さない。ほかの端末から送っていれば草はあるので、押せば見られる
- プロジェクト別は 5 件まで出し、残りは押すと出す。開くたびの Worker へのリクエストと D1 の読み取りを抑える
- 画像が読めなければ文言に置き換える。`<img>` は `loading="lazy"`、`referrerpolicy="no-referrer"`、775×200 を先に確保する
...
**このブラウザの記録も入れた** (2026-09-15、`src/userscript/render.ts`・`viewer.ts`、Issue #73)。

- `store.readRange` で直近 371 日を 1 回だけ読む (`bits` と `daily` を 1 回ずつ。日ごとの値は `readDay` と同じ)
- **寸法・色は共有 SVG と同じ `layoutGraph`** (§8) で決め、`createElementNS` で DOM の SVG にする。innerHTML も DOMParser も使わない
- **マスごとに `<title>` を付けてツールチップにする。** 文言は上の形で、記録の無い日は「記録なし」。まずブラウザ標準のツールチップにし、遅い・見づらいなら自前に替える
- 四分位とバランスの中心は、このブラウザの合算の 371 日から取り、プロジェクト別も同じスケールで塗る (ADR-0007 決定 4)
- 並びは合算、今のプロジェクト、残りは表示範囲の合計分数の多い順 (同じならコード単位の名前順)。プロジェクト別は 0.6 倍で、5 件まで
- **統合の方が未登録などで出せなくても、このブラウザの記録は出す**
- 今日の列はこのブラウザのローカル時刻の日付。共有 SVG は日本時間なので、日本以外ではずれうる
- 「ここから計測開始」の印はまだ無い (#80)

### 設定 UI

**「草の設定」のダイアログ** (2026-09-15、`src/userscript/settings.ts`・`settings-dialog.ts`、Issue #79)。
**入口は「草を見る」のダイアログの下のボタン** (2026-09-15、Issue #95。それまではページメニューの独立した項目だった)。
...
- **計測開始 = 記録のある最も古い日。** 別に「導入日」を持たない (localStorage にも D1 にも無く、
  新しく持つと端末間でずれる)。共有 SVG は `daily` の `ph = '*'` の全期間から、
  このブラウザの草は localStorage の記録から取る。**プロジェクト別も合算の開始日で塗り分ける**
  (そのプロジェクトを使い始めた日ではなく、計測していなかった期間を示すため)
- **開始日より前のマスは塗らず、点線の枠だけにする。** 何も描かないと欠けて見え、Level 0 の塗りとも
  区別が付かない。枠の色は文字色より薄い `#d0d7de` / `#3d444d`
- **ラベルは凡例の行の左に「点線は計測開始前」。** 当初の案は「グラフの左端にラベル」だったが、
  行を足すと高さが変わり、ダイアログが `<img>` で確保する寸法 (`GRAPH_WIDTH`/`GRAPH_HEIGHT`) まで動く。
  **寸法を変えずに済む凡例の行の空きに置いた**
...
- **「草を見る」で開いた草の URL は、scrapbox.io の Cache Storage に 48 時間残る** (Cosense の Service Worker が画像を保存する。research §3)。
