...
**2026-09-26 に決めた。実装は #151〜#154** (ADR-0021)。GitHub の Activity overview にならい、書いた分をどのページに書いたかで 3 つに分け、
...
- `c` と `o` は端末の中だけの 1440 bit のビットマップで、どちらも `w` の部分集合。**同じ分は `c` > `o` の順に 1 つにしか立てない**
  (作る > 関わる > 育てる)。サーバへは popcount だけを送る (ビットマップのままだと URL が §9 の上限を超える)
...
CREATE TABLE daily (
  ...
  wc INTEGER NOT NULL DEFAULT 0,       -- 作る (分)。ADR-0021、#151 で足す (未実装)
...
#### v2 (ADR-0021。#151 で実装する。いまはまだ v1)
...
- **`v=1` は 400。** 利用者がまだいないので両方は受けない (ADR-0021)。署名対象の正規化 (`src/shared/sign.ts`) の形はそのまま
...
**#151 で各日に `wc` / `wo` / `links` を足す** (ADR-0021)。育てるは足さない (`w − wc − wo` から出せる。0 で打ち切ることは §4)。
...
**書いたページの振り分け** (ADR-0021。#152 で実装する、未実装)。§4 の 4 軸のために、書いた分を作る・関わる・育てるに分ける。

- そのページで最初の `edit` が来たら、同一オリジンの `/api/pages/v2/:project/:title` を 1 回引く (`fetchText` の依存注入を使う)。
  自分の id は `/api/users/me` をドキュメントの寿命に 1 回だけ引く
- `user.id` が自分でなければ関わる、自分で `created` がローカルの今日なら作る、それより前なら育てる (research §4 の実測)。
...
- 作ると判定したページは、その日の `created` (ページ ID の集合) にも入れる。いまはセンサーが `created` を数えていない
- 同じ分に種類の違うページへ書いたら、`c` > `o` の順に 1 つにだけ立てる (`c` を立てるときは `o` を消し、`c` が立っていれば `o` を立てない)
...
- **v2 (ADR-0021、#151 で実装する)。** 各行に `c` (作る) と `o` (関わる) の base64url のビットマップと、`links` を足す。
  `c` と `o` は `w` の部分集合で、互いに重ならない。`links` は 1 日の中で (ページ ID, リンク先のハッシュ) の組で重複を除く集合で、
  リンク先の名前は持たない (#155)。**v1 の記録は読み捨てる** (利用者がまだいない。上の「知らない版は書かない」は新しい版から古い版を守る約束で、逆向きには当てない)
