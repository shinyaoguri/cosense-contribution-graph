...
src/userscript/
  ...
  purge.ts                  全データの削除 (署名して /v1/delete.gif を送り、ローカルの記録と鍵も消す)
...
**全削除も実装した** (2026-09-15、`src/worker/admin.ts`・`src/shared/purge.ts`・`src/userscript/purge.ts`、Issue #79)。

- **`confirm=1` は署名の対象にも入れる。** 値は `1` 以外を受けない
- **uid を持つ 5 つの表 (`daybits` / `daily` / `graphs` / `keys` / `enroll_tokens`) を 1 回の batch で消す。**
  batch はトランザクションなので、途中で失敗したら全部戻る
- **2 回目は 403 になる** (鍵も消えるので署名する鍵を引けない)。1 回目で消えているので実害は無い
...
- 送信の疎通確認 (Issue #31) の、タブを隠したときの自動送信は消した。メニューは残す
...
ページメニューに「草を見る」と「草の設定」を追加する。**「草を見る」は `<dialog>` で開く** (ADR-0003 の 2026-09-15 の改訂)。
...
- 全データの削除。**済み** (2026-09-15、Issue #79)。確認を挟んでから `/v1/delete.gif` を送り、ローカルの記録と鍵も消す
