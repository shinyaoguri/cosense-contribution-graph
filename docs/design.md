...
- **プロジェクト名は凡例の行の左端に太字で出す** (2026-09-18、Issue #119)。
  計測開始の注記があるときは、名前 → 注記の順に左から並べる。
  **名前の幅は測れないので概算で送る** (9px の半角英数 1 文字 = 5.5px)。凡例は右寄せで遠いので、
  多少ずれても間隔が変わるだけ。**ここに置くのは寸法を増やさないため** (下と同じ理由)
