...
- **`<img>` 内の SVG は完全に非インタラクティブ。** `<title>` のツールチップも出ず (草のダイアログのこのブラウザの記録は、ページの中の SVG なので出る。§9)、外部フォントも
...
**統合の草が主で、このブラウザの記録は内訳** (2026-09-15、Issue #101。ADR-0003 の改訂)。

- **全端末を統合した記録** — 共有 SVG を `img` で埋め込む。非インタラクティブだが全デバイス分が見られる。**これを先に出す**
- **このブラウザの記録** — localStorage から描く。ツールチップ付きでインタラクティブ。**`<details>` に畳む**。
  合算の草を上に 1 枚、下にプロジェクト別の草を小さく並べる。
  **統合の草を出せないとき (未登録など) と、まだ 1 件も送れていないときは開いた状態で出す** (見るものがこれしか無い)

当初は 2 つを対等な節として並べていたが、どちらも合算とプロジェクト別を並べるので同じ形の草が最大 12 枚縦に並び、
値が違うときにどちらが本当かが読めなかった。**畳んでも中身は作る** — 今までも毎回描いていたので重くならず、
...
- **統合の方が未登録などで出せなくても、このブラウザの記録は出す**。
  **`<details>` に畳んだ後も同じで、そのときは開いた状態で出す** (2026-09-15、Issue #101)
