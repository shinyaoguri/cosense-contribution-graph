...
| 日次の集計値 (`daily`) | **最後の送信から 2 年。** Cron で削除する |
| 公開鍵 (`keys`) | 失効させるか、利用者のデータ全体を削除するまで |

`daily` を無期限に持つと D1 Free の 500MB に当たる。1 人 1 年で
`365 × (プロジェクト数 + 1)` 行なので、3 プロジェクトなら 1,460 行。3,000 人で 10 年分を
持つと枠を超える。

**利用者に告知する手段がない。** メールアドレスを保存していないので、削除を予告できない。
予告なく消すことをプライバシーポリシーに明記する。2 年使っていないなら、草も見ていない。
